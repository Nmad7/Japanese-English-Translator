...
class ai_translator:
    def __init__(self):
        self.model = load_model('cp_finalApproach-14.hdf5')
        self.model.compile(loss = 'sparse_categorical_crossentropy',optimizer = Adam(.001),metrics = ['accuracy'])
        with open('ja_tokenizer3.pickle', 'rb') as handle:
            self.ja_tokenizer = pickle.load(handle)
        with open('eng_tokenizer3.pickle', 'rb') as handle:
